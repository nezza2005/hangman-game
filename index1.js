...
const app = express();
//create request and response from server.
app.get("/", function(req, res) {
  responce.send(randomWords())
})
//create port.
function portListen(e) {
  console.log("Server started on port 3000!") // just to test that listner works in node/hyperterminal.
}


//install NPM external module for random words.
let randomWords = require("random-words");
let chosenWords = randomWords({ min: 1, max: 3 });
...
var randomiseWordHide = chosenWords.replace(/[a-z]/gi, '*'); // regular expression using regexp to find only a-z characters and replace with *.
...
//if correct guess, reveal characters,
  ...
  //if incorrect guess, push to array to keep count. keep this out of the loop.
    if (!chosenWords.includes(event.key)) {
...
function endResult() {
  ...
  if (guessCount.textContent.slice(-1) <= 5 && solved) {
    ...
    document.querySelector("#guesses, p").textContent = "Nice try, the word was " + '"' + chosenWords.toUpperCase() + '".';
...
//listens to a specified port for any http requests.
app.listen(3000, portListen);
